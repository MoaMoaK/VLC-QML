...
MainInterface::MainInterface( intf_thread_t *_p_intf ) : QVLCMW( _p_intf )
{
    ...
    b_hideAfterCreation  = false; // --qtqml-start-minimized
    ...
    setWindowOpacity( var_InheritFloat( p_intf, "qtqml-opacity" ) );
#ifdef Q_OS_MAC
    ...
    b_autoresize = var_InheritBool( p_intf, "qtqml-video-autoresize" );

    /* Are we in the enhanced always-video mode or not ? */
    b_minimalView = var_InheritBool( p_intf, "qtqml-minimal-view" );

    /* Do we want anoying popups or not */
    i_notificationSetting = var_InheritInteger( p_intf, "qtqml-notification" );

    /* */
    b_pauseOnMinimize = var_InheritBool( p_intf, "qtqml-pause-minimized" );
    ...
    if( var_InheritBool( p_intf, "qtqml-name-in-title" ) )
    {
        CONNECT( THEMIM->getIM(), nameChanged( const QString& ),
...
void MainInterface::reloadPrefs()
{
    i_notificationSetting = var_InheritInteger( p_intf, "qtqml-notification" );
    b_pauseOnMinimize = var_InheritBool( p_intf, "qtqml-pause-minimized" );
    if( !var_InheritBool( p_intf, "qtqml-fs-controller" ) && fullscreenControls )
    {
        delete fullscreenControls;
        fullscreenControls = NULL;
    }
...
void MainInterface::showResumePanel( int64_t _time ) {
    int setting = var_InheritInteger( p_intf, "qtqml-continue" );
...
void MainInterface::onInputChanged( bool hasInput )
{
    if( hasInput == false )
        return;
    int autoRaise = var_InheritInteger( p_intf, "qtqml-auto-raise" );
...
void MainInterface::createMainWidget( QSettings *creationSettings )
{
    ...
    if ( QDate::currentDate().dayOfYear() >= QT_XMAS_JOKE_DAY
         && var_InheritBool( p_intf, "qtqml-icon-change" ) )
    {
    ...
    if ( !var_InheritBool( p_intf, "qtqml-bgcone" ) )
        bgWidget->setWithArt( false );
    else
        if ( var_InheritBool( p_intf, "qtqml-bgcone-expands" ) )
    ...
    if ( depth() > 8 ) /* 8bit depth has too many issues with opacity */
        ...
        if( var_InheritBool( p_intf, "qtqml-fs-controller" ) )
        {
            fullscreenControls = new FullscreenControllerWidget( p_intf, this );
            CONNECT( fullscreenControls, keyPressed( QKeyEvent * ),
                     this, handleKeyPress( QKeyEvent * ) );
...
inline void MainInterface::initSystray()
{
    ...
    bool b_systrayWanted = var_InheritBool( p_intf, "qtqml-system-tray" );

    if( var_InheritBool( p_intf, "qtqml-start-minimized") )
    {
...
void MainInterface::setVideoFullScreen( bool fs )
{
    ...
    if( fs )
    {
        int numscreen = var_InheritInteger( p_intf, "qtqml-fullscreen-screennumber" );
